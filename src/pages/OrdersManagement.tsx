...
const OrdersManagement = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1600px] mx-auto pb-12 px-4 sm:px-6">
      {/* Premium Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md rounded-2xl border border-zinc-200/50 dark:border-zinc-800/40 p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between lg:justify-start gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-black text-zinc-900 dark:text-white leading-tight">Kitchen Screen</h1>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-widest font-black mt-1 flex items-center gap-2">
                ...
                {pendingCount > 0 ? (
                  ...
                  <span className="text-zinc-500 font-semibold">No Pending Orders</span>
                )}
              </p>
            </div>
          </div>
          
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold shadow-sm">
        ...
        <div className="flex gap-3 shrink-0 w-full lg:w-auto">
          <div className="relative flex-1 lg:w-72">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 size-4 text-zinc-450 dark:text-zinc-500" />
            <Input
          ...
          <Button
            ...
            size="icon"
            onClick={fetchOrders}
            className="size-12 rounded-xl shrink-0 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 cursor-pointer transition active:scale-95 shadow-sm"
          >
        ...
        {STATUS_TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                'shrink-0 flex flex-col items-center min-w-[105px] px-6 py-4 rounded-2xl border-2 font-bold transition-all duration-200 active:scale-[0.96] cursor-pointer shadow-sm',
                tabColors
              )}
            >
          ...
          {filteredOrders.map((order) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'bg-white dark:bg-zinc-900/95 rounded-2xl overflow-hidden flex flex-col shadow-sm border transition-all duration-300 cursor-pointer hover:shadow-lg hover:scale-[1.01]',
      ...
      <Sheet open={selectedOrder !== null} onOpenChange={(open) => { if (!open) setSelectedOrder(null); }}>
        <SheetContent side="right" className="w-full sm:max-w-md md:max-w-lg border-l border-zinc-200 dark:border-zinc-800 flex flex-col p-0 bg-zinc-50 dark:bg-zinc-950 overflow-hidden shadow-2xl">
